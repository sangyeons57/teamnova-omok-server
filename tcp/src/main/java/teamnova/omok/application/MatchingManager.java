...
import teamnova.omok.store.MatchingStore;
...
public class MatchingManager {
    ...
    public void start(MatchingStore store) {
        store.getMatching().createGroup(2);
        store.getMatching().createGroup(3);
        store.getMatching().createGroup(4);
    }

    public MatchingResult tryMatch(MatchingStore store) {
        Ticket ticket = store.getMatching().poll();
        ...
        for (int match : ticket.matchSet) {
            Group group = buildGroup(getNeighborTickets(store, match, ticket));
        ...
        if (bestGroup != null){
            ...
            for (Ticket t : bestGroup.getTickets()) store.dequeue(t);
    ...
    private List<Ticket> getNeighborTickets(MatchingStore matching, int match, Ticket selectedTicket) {
