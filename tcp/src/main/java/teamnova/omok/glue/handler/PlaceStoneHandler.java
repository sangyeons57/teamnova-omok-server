...
public class PlaceStoneHandler implements FrameHandler {
    ...
    public void handle(NioReactorServer server, ClientSessionHandle session, FramedMessage frame) {
        ...
        if (comma < 0) {
            session.sendPlaceStoneError(frame.requestId(), "INVALID_PAYLOAD");
            return;
        }
        int x, y;
        try {
            ...
            session.sendPlaceStoneError(frame.requestId(), "INVALID_COORDINATES");
            return;
        }

        boolean accepted = session.submitMove(frame.requestId(), x, y);
        if (!accepted) {
            session.sendPlaceStoneError(frame.requestId(), "SESSION_NOT_FOUND");
        }
    }
}
